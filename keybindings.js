...
function registerNavigatorAction(name, handler) {
  ...
  registerAction(name, handler, { settings: settings, opensNavigator: true });
}
...
function init() {
  ...
  registerAction("live-alt-tab", liveAltTab, { settings });
  registerAction("live-alt-tab-backward", liveAltTab, {
    settings,
  ...
  registerMinimapAction("move-left", (mw, space) =>
    space.swap(Meta.MotionDirection.LEFT)
  );
  registerMinimapAction("move-right", (mw, space) =>
    space.swap(Meta.MotionDirection.RIGHT)
  );
  registerMinimapAction("move-up", (mw, space) =>
    space.swap(Meta.MotionDirection.UP)
  );
  registerMinimapAction("move-down", (mw, space) =>
    space.swap(Meta.MotionDirection.DOWN)
  );

  registerPaperAction(
    "toggle-scratch-window",
    dynamic_function_ref("toggleScratchWindow", Scratch)
  );

  registerPaperAction(
    "toggle-scratch-layer",
    dynamic_function_ref("toggleScratch", Scratch)
  );

  registerPaperAction(
    "toggle-scratch",
    dynamic_function_ref("toggle", Scratch),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "develop-set-globals",
    dynamic_function_ref("setDevGlobals", Utils)
  );

  registerPaperAction(
    "resize-h-inc",
    dynamic_function_ref("resizeHInc", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "resize-h-dec",
    dynamic_function_ref("resizeHDec", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "resize-w-inc",
    dynamic_function_ref("resizeWInc", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "resize-w-dec",
    dynamic_function_ref("resizeWDec", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "cycle-width",
    dynamic_function_ref("cycleWindowWidth", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "cycle-height",
    dynamic_function_ref("cycleWindowHeight", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "center-horizontally",
    dynamic_function_ref("centerWindowHorizontally", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );
  ...
  registerPaperAction(
    "slurp-in",
    dynamic_function_ref("slurp", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "barf-out",
    dynamic_function_ref("barf", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );

  registerPaperAction(
    "toggle-maximize-width",
    dynamic_function_ref("toggleMaximizeHorizontally", Tiling),
    Meta.KeyBindingFlags.PER_WINDOW
  );
  ...
  registerPaperAction(
    ...
    (metaWindow) => {
      if (metaWindow.fullscreen) metaWindow.unmake_fullscreen();
      else metaWindow.make_fullscreen();
    },
    Meta.KeyBindingFlags.PER_WINDOW
  );
}
...
var asKeyHandler;
if (global.screen) {
  asKeyHandler = (actionHandler) => (display, screen, mw, binding) => {
    return actionHandler(mw, Tiling.spaces.selectedSpace, {
      display,
      screen,
      binding,
    });
  ...
  asKeyHandler = (actionHandler) => (display, mw, binding) => {
...
function registerAction(actionName, handler, options) {
  ...
  let { settings, opensNavigator } = options;
...
function devirtualizeMask(gdkVirtualMask) {
  ...
  if (!success) throw new Error("Couldn't devirtualize mask " + gdkVirtualMask);
  return rawMask;
}

function rawMaskOfKeystr(keystr) {
  let [dontcare, keycodes, mask] = Gtk.accelerator_parse_with_keycode(keystr);
...
function enableAction(action) {
  ...
  if (action.options.settings) {
    ...
    if (display.grab_accelerator.length > 1) {
      actionId = display.grab_accelerator(
        action.keystr,
...
function overrideAction(mutterName, action) {
  ...
  if (id === Meta.KeyBindingAction.NONE) return;
...
function resetConflicts() {
  ...
  for (let name of names) {
    ...
    if (
      name.startsWith("switch-to-workspace-") ||
    ...
    switch (name) {
      case "cycle-group":
      case "cycle-group-backwards":
      case "cycle-windows":
      case "cycle-windows-backwards":
