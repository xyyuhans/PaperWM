...
class Minimap extends Array {
  constructor(space, monitor) {
    ...
    container.height =
      Math.round(space.height * MINIMAP_SCALE) - prefs.window_gap;
    ...
    clip.set_position(
      12 + prefs.window_gap,
      12 + Math.round(1.5 * prefs.window_gap)
    );
  ...
  _allocateClone(container) {
    ...
    clone.set_size(
      buffer.width * MINIMAP_SCALE,
      buffer.height * MINIMAP_SCALE - prefs.window_gap
    );
    clone.set_position(
      (buffer.x - frame.x) * MINIMAP_SCALE,
      (buffer.y - frame.y) * MINIMAP_SCALE
    );
    container.set_size(
      frame.width * MINIMAP_SCALE,
      frame.height * MINIMAP_SCALE
    );
  }
  ...
  select() {
    ...
    highlight.y = Math.round(clip.y + selected.y - prefs.window_gap);
    highlight.set_size(
    ...
    label.set_position(Math.round(x), clip.y + Math.round(clip.height + 20));
